...
async def get_judge_result(
    ...
    try:
        ...
        # 傳送檔案
        local_file = similar_file("hw8-2.c", judge.local_dir_path)
        await judge.upload(
            local_file=local_file,
            remote_file="hw8-2.c"
        )
        local_file = similar_file("hw8-2.h", judge.local_dir_path)
        await judge.upload(
            local_file=local_file,
            remote_file="hw8-2.h"
        )
        ...
        # 題目檔案
        await judge.write(MAIN_CODE, "main2.c")

        stdout, stderr, return_code = await judge.command(
            "gcc main2.c hw8-2.c -o hw8-2",
            timeout=5,
            chdir=True
        )
        if return_code != 0:
